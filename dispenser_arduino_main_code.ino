...
#define laserEmitter 6        //Laser diode control pin
#define laserSensor 8         //Laser reciever control pin

#define ultrasonicIn 7       //Ultrasonic sensor echo input
...
const int dispSpeed = 125;  //analog    How fast the mask is dispensed, from 0 to 255
...
void loop() {
  ...
  Serial.println(digitalRead(laserSensor));
...
void MeasureDistance(){
  ...
  // Serial.println(distance);                                           //We know the distance from the sensor to the front edge of the dispenser.
...
void DispenseMask(){
  ...
  if(digitalRead(laserSensor) == HIGH){  //If there is something blocking the laser reciever (i.e. a mask)
